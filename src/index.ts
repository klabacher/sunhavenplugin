...
function pathPattern(api, game, pattern) {
  const gamePath = api.getState().settings.gameMode.discovered[game.id]?.path;
  const documents = util.getVortexPath('documents');
  const localAppData = process.env['LOCALAPPDATA'];
  const appData = util.getVortexPath('appData');

  const template = fs.readFileSync(pattern, 'utf-8');
  const replacedTemplate = template.replace(/\$\{gamePath\}/g, gamePath)
    .replace(/\$\{documents\}/g, documents)
    .replace(/\$\{localAppData\}/g, localAppData)
...
function prepareForModding(discovery, api) {
  const ModPath = path.join(discovery.path, 'BepInEx', 'plugins');
  if (!fs.ensureDirWritableAsync(ModPath)) {
    fs.mkdirAsync(ModPath);
  }
  return true
}
